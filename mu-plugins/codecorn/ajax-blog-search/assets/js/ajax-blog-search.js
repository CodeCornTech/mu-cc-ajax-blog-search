...
jQuery(function ($) {
    ...
    var AJX_CLP_DBG = (function () {
        ...
        if (typeof window.CC_Ajax_Blog_Search !== 'undefined' && typeof CC_Ajax_Blog_Search.debug !== 'undefined') {
        ...
        $(selectors.join(',')).each(function () {
            ...
            function renderResults(items, term) {
                ...
                if (!items || !items.length) {
                    $resultsBox.html('<div class="cc-ajax-search-empty">' + (CC_Ajax_Blog_Search.no_results_text || 'Nessun articolo trovato .') + '</div>');
                    showBox();
                    return;
                }

                var html = '<div class="cc-ajax-search-heading">Risultati per "<strong>' + $('<div>').text(term).html() + '</strong>"</div>';
                ...
                items.forEach(function (item, idx) {
                    ...
                    if (hasThumb) {
                        ...
                        html += '</span>';
                        if (item.date) {
                            html += '<span class="cc-ajax-search-date cc-ajax-search-date--under-thumb">' + item.date + '</span>';
                        }
                        ...
                        if (item.excerpt) {
                            html += '<span class="cc-ajax-search-excerpt cc-ajax-search-excerpt--full">';
                            html += item.excerpt;
                            html += '</span>';
                        }
                    } else {
            ...
            function renderError() {
                ...
                $resultsBox.html('<div class="cc-ajax-search-error">' + (CC_Ajax_Blog_Search.error_text || 'Errore nella ricerca .') + '</div>');
                showBox();
    ...
    function initSidebarToggle() {
        ...
        if (typeof CC_Ajax_Blog_Search === 'undefined' || !CC_Ajax_Blog_Search.sidebar_toggle || !CC_Ajax_Blog_Search.sidebar_toggle.enabled) {
        ...
        var $placeholder = $('<div class="cc-sidebar-placeholder" style="display:none;"></div>');
        ...
        var $close = $('<button type="button" class="cc-sidebar-panel__close" aria-label="Chiudi filtri">&times;</button>');
        ...
        var $toggle = $('<button type="button" class="' + toggleClass + '"></button>');
        $toggle.text(label);
        $('body').append($toggle);
